...
- 部署平台：Vercel Hobby（免费层）
- 仓库策略：前后端双仓分离
  - 前端仓库（公开）：部署至 Vercel 项目名 `tarot`
  - 后端仓库（私有）：部署至 Vercel 项目（推荐名）`tarot-backend`
- 安全原则：
  - 前端不注入任何敏感密钥；仅包含非敏感配置（如模型名、后端基座地址）
  - 后端持有所有敏感密钥（如 `GEMINI_API_KEY`），通过 Serverless 路由承接 AI 请求
  - CORS 白名单：后端仅允许来自前端正式域名的请求
- 体验一致性：
  - 开发模式启用本地代理（`VITE_AI_DEV_PROXY=1`），接口形态与生产一致（均走 `/api/ai/...`）
  - 生产模式关闭前端代理（`VITE_AI_DEV_PROXY=0`），通过后端路由调用 AI 服务
  - 产品交互与呈现保持一致，避免“开发与生产行为不一致”的体验差
...
const questionTemplates = {
  love: ['我和TA的关系会如何发展？', '如何改善我们之间的沟通？', '我应该如何面对这段感情？'],
  career: ['我的职业发展方向是什么？', '现在是换工作的好时机吗？', '如何提升我的工作能力？'],
  finance: ['我的财运状况如何？', '这个投资决定是否明智？', '如何改善我的财务状况？'],
  fortune: [
    '我近期的整体运势如何？',
    '什么会给我带来好运？',
    '我应该注意哪些运势变化？',
  ...
  personal: ['我应该如何面对当前的困境？', '什么阻碍了我的成长？', '我的内心真正渴望什么？'],
};
```
...
const dataProcessingPipeline = {
  ...
  validateData: (cards) => {
    const isValid =
      cards.length === 78 &&
      cards.every((card) => card.uniqueId && card.name && typeof card.isReversed === 'boolean');
...
const dataConsistencyChecks = {
  ...
  afterInitialization() {
    ...
    console.assert(
      cards.every((card) => typeof card.isReversed === 'boolean'),
      '正逆位状态未正确设置',
    );
  },
  ...
  afterCardSelection(selectedCard) {
    ...
    console.assert(cachedCard.isReversed === selectedCard.isReversed, '选中卡牌与缓存数据不一致');
  },

  // 检查点3: 解读生成前
  beforeReadingGeneration(cardForReading) {
    console.assert(cardForReading.id && cardForReading.name, '传递给解读引擎的卡牌数据不完整');
    console.assert(typeof cardForReading.isReversed === 'boolean', '卡牌正逆位状态未定义');
  },
};
```
...
目标
- 在保证体验与稳定的前提下，避免在前端暴露任何第三方 API 密钥
- 为未来国内/海外场景留出可演进空间

一、推荐部署拓扑（面向生产）
- 前端：Public 仓库 + 静态托管（GitHub Pages/Cloudflare Pages/任意 CDN）
- 后端：Private 仓库 + 海外区域运行平台（Cloud Run/Render/Fly.io/Cloudflare Workers 等）
- 前端仅调用自有后端域名；后端统一代理第三方（Gemini/Tarot），做鉴权、重试、限流、内容安全与缓存

二、仓库与权限
- 前端仓（Public）：主分支保护、PR 合并、CI 门禁、部署凭据最小权限
- 后端仓（Private）：集中管理 Secrets 与 CI/CD；降低攻击面并满足审计/合规

三、密钥与安全
- 原则：密钥只存在服务端/平台 Secret 管理，不下发到浏览器
- 风险：泄露可能导致配额被滥用/扣费/封禁，即便免费套餐亦会影响可用性
- 控制：后端代理、分环境密钥与轮换、预算告警、来源/IP 白名单（若支持），疑似泄露即时停用

四、区域与连通性
- 海外区域（如新加坡/东京/硅谷）可直接访问 Google 生态，适合调用 Gemini
- 中国大陆区域云主机通常无法直连 Google；若主要面向国内用户，可将后端放置于香港/新加坡，或切换兼容的国内模型
- 用户只需访问前端/后端域名，无需直连第三方；外网连通性由后端承担

五、Tarot 与 AI 路由策略
- Tarot：现阶段公共接口无需密钥，前端内置本地牌面数据作兜底；生产可通过后端转发并加缓存
- AI（Gemini）：生产一律走后端代理，集中实现鉴权、重试/熔断/限流与内容安全

六、阶段性决策
- 当前阶段（功能验证/MVP）：沿用现有实现以验证体验，开启失败回退路径，保证不中断
- 生产阶段：切换“前端 Public + 后端 Private + 代理第三方”的拓扑；密钥仅在后端

七、风险与备选
- 预算与配额：设置上限与告警；出现异常请求量时自动熔断
- 区域风险：准备“国内模型适配层”作为替代路径
- 性能风险：前端缓存不敏感数据、后端开启缓存与压缩
