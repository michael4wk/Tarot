...
#### 1.3 图片资源映射策略
```javascript
// 统一的图片资源映射策略（禁止字符串拼接路径）
// 在构建期由 Vite 的 import.meta.glob 收集资源清单，运行期必须通过工具函数获取路径
import { getCardImagePath, attachFrontSrc } from '@/utils/images'

// 单张卡牌路径获取（推荐）
const frontSrc = getCardImagePath({
  type: card.type,                       // 'major' | 'minor'
  suit: card.suit ?? 'major',            // 大阿卡纳使用 'major'
  value: card.value ?? card.name_short   // 大阿卡纳用标准化 value/别名，例如 'fool'
})

// 批量生成 frontSrc（推荐，用于列表/抽牌返回值）
const cardsWithSrc = attachFrontSrc(cards)

// 卡背资源（由工具函数或统一常量提供，避免硬编码）
const cardBack = CARD_BACK_SVG
```
> 注意：严禁出现诸如 `/assets/images/cards/major_arcana_${card.name_short}.png` 的路径拼接写法，避免与构建产物路径不一致导致 404。
