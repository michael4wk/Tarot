...
describe('withReversal', () => {
  ...
  it('preserveExisting=true 时保留已存在的 isReversed', () => {
    const input = [{ id: 1, isReversed: true }, { id: 2, isReversed: false }]
    const out = withReversal(input, { reversedProbability: 0.0, random: mulberry32(1), preserveExisting: true })
  ...
  it('preserveExisting=false 时可覆盖已存在的 isReversed', () => {
    const input = [{ id: 1, isReversed: true }, { id: 2, isReversed: false }]
    const out = withReversal(input, { reversedProbability: 1.0, random: mulberry32(1), preserveExisting: false })
    expect(out[0].isReversed).toBe(true)
